...
try:
    ...
    os.system('cmd /c "python -m pip install tkinter"') #execute the command (a string) in a subshell.
    from tkinter import *
try:
    import tkcalendar 
except:
...
def check_table(*args, **kwargs):
    try:
        ...
        return "DB ERROR"
    tables=[]
    ideal= ['id', 'Book_name', 'stock', 'cp', 'sp', 'totalcp', 'totalsp', 'assumed_profit', 'vendor', 'date', 'time']
    try:
...
def main(entry, *args, **kwargs):     #first entry with database deatails; getting all details by user by entry widget
    ...
    title= Label(entry, text="Welcome to RNS BOOK STORE", font=("Stylish",21, "bold"), bg="beige", fg="#EE8262")
    ...
    def else_load(*args, **kwargs):
        ...
        def create_fun(*args, **kwargs):
            def createe(*args, **kwargs):
                ...
                try:
                    ...
                    c.execute(f"USE {eee}")
                    c.execute(f"CREATE TABLE inventory (ID INT PRIMARY KEY auto_increment, Book_Name VARCHAR(100) NOT NULL, stock INT NOT NULL, cp INT NOT NULL, sp INT NOT NULL, totalcp INT, totalsp INT, assumed_profit INT, Vender VARCHAR(100), date Varchar(12), time Varchar(12));")
                    conn.commit()
                except:
            ...
            entry.title("RNS BOOK STORE.create_database")
            db_e.delete(0, END)
            db_e.focus()
        ...
        def load_fun(*args, **kwargs):
            ...
            entry.title("RNS BOOK STORE.load_database")
            deff.destroy()
            ...
            title= Label(fr, text="RNS BOOK STORE", font=("Comic Sans", 22, "bold"), bg="beige", fg="#EE8262")
            title.place(x=30, y=10)
...
#ENTRY WINDOW    
entry=Tk()
entry.title("RNS BOOK STORE.enter")  #putting title name of window
...
def stock_addition(*args, **kwargs):     #stock addition section to add data in the db
    ...
    class Stock_addition:           #running Stock_addition class when stock_addition button is called
        def __init__(self, root,*args, **kwargs):
            root1.title("RNS BOOK STORE-.Addition")
            try:
            ...
            def back(*args, **kwargs):
                root1.title("RNS BOOK STORE.Billing")
            ...
            if self.break_id!=0:
                ...
                self.ID_l= Label(master, text='Book ID will be {}'.format(self.break_id), font=("arial 15 bold"), fg="tomato", bg="#88BDBC")
                self.ID_l.place(x=100,y=80)
            else:
                #labels for the window
                self.ID_l= Label(master, text='Book ID will be {}'.format(int(ids_list[-1])+1), font=("arial 15 bold"), fg="tomato", bg="#88BDBC")
                self.ID_l.place(x=100,y=80)
            global ID_l
            ID_l= self.ID_l
            self.name_l= Label(master, text='Enter Book Name ', font=("arial 19 bold"), bg="#88BDBC").place(x=10,y=130)
        ...
        def get_items(self, *args, **kwargs):
            ...
            if self.stock.isdigit() and self.cp.isdigit() and self.sp.isdigit(): 
                if self.notcon!=0:
                    ...
                    cur.execute(f"select Book_Name from {TableName};")
                    namess=[]
                    for i in cur:
                    ...
                    if self.name.lower() not in namess:
                        ...
                        query= "INSERT INTO {} (id, Book_Name, stock, cp, sp, totalcp, totalsp, assumed_profit, Vender, date, time) VALUES ({},'{}',{},{},{},{},{},{},'{}','{}','{}');".format(TableName,ids_list[-1]+1, self.name, self.stock, self.cp, self.sp, self.totalcp, self.totalsp, self.assumed_profit, self.vendor, date, time)
                        cur.execute(query)
                        self.conn.commit()
                        tkinter.messagebox.showinfo("Success", "You have successfully added one item to the database!!!") 
                        get_ids()
                        self.ID_l.configure(text='Book ID will be {}'.format(ids_list[-1]+1))
                        self.clear()
                        self.name_e.focus()
                    else:
                        if tkinter.messagebox.askyesno("Error", f"{self.name} already exists!\nDo you want to add stock data with old details?"):
                            cur.execute(f"select id, stock, totalcp, totalsp, assumed_profit from {TableName} where Book_Name='{self.name}';")
                            a=cur.fetchall()[0]
                    ...
                    if self.name.lower() not in namess:
                        date=datetime.datetime.now().strftime("%d.%m.%Y")
                        time=datetime.datetime.now().strftime("%H:%M:%S")
                        query= "INSERT INTO {} (id, Book_Name, stock, cp, sp, totalcp, totalsp, assumed_profit, Vender, date, time) VALUES ({},'{}',{},{},{},{},{},{},'{}','{}','{}');".format(TableName,self.break_id, self.name, self.stock, self.cp, self.sp, self.totalcp, self.totalsp, self.assumed_profit, self.vendor, date, time)
                        cur.execute(query)
                        self.conn.commit()
                        tkinter.messagebox.showinfo("Success", "You have successfully added one item to the database!!!") 
                        get_ids()
                        ...
                        if self.notcon!=0:
                            ...
                            self.ID_l.configure(text='Book ID will be {}'.format(self.break_id ))
                            self.clear()
                        self.name_e.focus()
                    else:
                        if tkinter.messagebox.askyesno("Error", f"{self.name} already exists!\nDo you want to add stock data with old details?"):
...
def stock_modify(*args, **kwargs):     #stock modification section to modify data in the db
    ...
    class Stock_modify:                  #running Stock_Modify class when stock_modify button is called
        def __init__(self, root,*args, **kwargs):
            root1.title("RNS BOOK STORE.Modification")
            self.conn= sql.connect(host="localhost", user=uname, passwd=pwd, database=dbName)
            canvas= Frame(root, width=765, height=600, bg="#88BDBC")
            ...
            #labels for the window
            self.searchl= Label(master, text="Select Book ID", font=("arial 19 bold"), bg="#88BDBC").place(x=22,y=80)

            self.name_l= Label(master, text='Book Name ', font=("arial 19 bold"), bg="#88BDBC").place(x=22,y=200)
            ...
            val=[]
            c= self.conn.cursor()
            c.execute("select id, Book_Name from inventory")
            for i in c.fetchall():
        ...
        def search(self, *args, **kwargs):
            try:
                ...
                for i in cur:
                    lii.append(i[0])
                if self.searche=="":
                    tkinter.messagebox.showerror("Error", "Kindly enter Book ID")
                    self.clear()
                elif self.searche in lii:
                    def prr(*args, **kwargs):
                        self.textt.place(x=120,y=150)
                        self.textt.config(text="Book ID: {}".format(self.searche))
                    ...
                    tkinter.messagebox.showerror("Error!!!", "Book ID not present in database \nYou can add the Book from the stock addition section.")
                    self.clear()
                    self.textt.place_forget()
        ...
        def modify_items(self, *args, **kwargs):
            ...
            if self.name!='' or self.stock!='' or self.cp!='' or self.sp!='':
                cur=self.conn.cursor()
                if self.a!=self.name:
                    query="UPDATE {} SET Book_name='{}' WHERE ID={}".format(TableName,self.name, self.idd)
                    cur.execute(query)
                    self.conn.commit()
...
def stock_review(*args, **kwargs):     #inside the stock addition section
    class Stock_review:                #running Stock_review class when stock_review button is called
        def __init__(self, root,*args, **kwargs):
            root1.title("RNS BOOK STORE.Stock_Review")
            self.conn= sql.connect(host="localhost", user=uname, passwd=pwd, database=dbName)
            ...
            def callback(selection, *args, **kwargs):
                ...
                elif selection=="By Vendor":
                    sortbyoptions.place_forget()
                    c.execute(f"select Vendor from {TableName}")
                    ll1.config(text="Sort By Vendor")
                    dump_btn.place_forget()
                    val=[]
                    for x in c:
                        if x[0] not in val:
            ...
            sortbyoptions.place(x=120, y=15)
            #Book deleting
            self.delete_btn= Button(master, text="Delete Book",font=('ariel', 14, 'bold'), bg="lightgreen", command=self.delete)
        ...
        def setup(self, *args, **kwargs):
            ...
            self.trees.heading("name", text="Book Name", anchor=W)
...
def sold_item_review(*args, **kwargs):      #inside the stock review section
    class Sold_item_review:                 #running Sold_item_review class when sold_item_review button is called
        def __init__(self, root,*args, **kwargs):
            root1.title("RNS BOOK STORE.Sold_Item_Review")
            self.conn= sql.connect(host="localhost", user=uname, passwd=pwd, database=dbName)
            ...
            def show_bill(*args, **kwargs):
                ...
                if selc!=():
                    ...
                    try:
                        ...
                        return
                    file.insert(-7, "\n")
                    txt.insert(END,"\t\tRNS BOOK STORE\n")
                    txt.insert(END, "\t\t   TAMILNADU-India\n")
                    txt.insert(END, "\t\t phone-9363263500\n")
                    txt.insert(END, "\t\t\tInvoice\n\n")
                    txt.insert(END, f"Date:{date}\t\t\t\tTime-{time}")
                    txt.insert(END, '''
================================================
Sno. Book Name			    Qty	Amount
================================================\n''')
                    for i in file:
                        txt.insert(END, i)
...
class Billing:
    def __init__(self,root1,left, right, *args, **kwargs):
        ...
        if "transactions" not in tbs:
            c.execute("Create table transactions(ID INT , Book_Name VARCHAR(100),Quantity INT,Received_Amount INT, bill_profit INT, date VARCHAR(12), time VARCHAR(12), invoice INT(10));")
            self.conn.commit()
        else:
            ...
            for i in c.fetchall():
                desc.append(i[0])
            ideal=["ID", "Book_Name", "Quantity", "Received_Amount", "bill_profit", "date", "time", "invoice"]
            if ideal==desc:
                pass
            else:
        ...
        clock()
        headl= Label(self.left, text= "RNS BOOK STORE" ,  font=("arial 30 bold"), bg="#88BDBC", fg="darkblue")
        headl.place(x=10, y=90)
        headb= Label(self.left, text= "Billing" ,  font=("arial 28 bold"), bg="#88BDBC", fg="#B40505")
        headb.place(x=175, y=136)
        self.id= Label(self.left, text= "Select Book ID" ,  font=("arial 20 bold"), bg="#88BDBC")
    ...
    def searchid(self, *args, **kwargs):
        ...
        try:
            if int(self.id_e) in self.idss:
                ...
                q1= "select Book_Name, stock, sp, vender, cp from inventory where id={};".format(self.id_e)
                cur.execute(q1)
                for i in cur:
                    self.get_name= i[0]
                    self.get_stock= i[1]
                    ...
                    self.get_vender=i[3]
                    self.get_cp= i[4]
                    self.pid= Label(self.showinfo, text= "Book ID {}".format(self.id_e), font=("arial 17 bold"),fg="#3A00DE",bg="#88BDBC")
                    self.pid.place(x=10, y=10)
                    self.pn.configure(text="Book Name: {}".format(i[0]))
    ...
    def gbill(self, *args,**kwargs):
        if len(prodquan)==0:
            ...
            def submit_fun(*args, **kwargs):
                ...
                div="\t\t   ================================================\n"
                header="\t\t    Sno. Book Name\t\t\t Qty\tAmount\n"
    ...
    def detuptst(self, index,*args, **kwargs):
        ...
        c= self.conn.cursor()
        c.execute(f"select stock from {TableName} where Book_Name='{name}';")
        st=""
        for i in c:
            st= i[0]
        newst= int(st)+ int(quan)
        c.execute(f"update {TableName} set stock={newst} where Book_Name='{name}';")
        self.conn.commit()
        self.uptstock-=1
...
#adding infotext
tip = tkinter.tix.Balloon(root1)
tip.bind_widget(headbtn1, balloonmsg = "Here you can add new Books")
tip.bind_widget(headbtn2, balloonmsg = "Here you can modify the details\nof existing Books.")
tip.bind_widget(headbtn3, balloonmsg = "Here you can view all the Books\n and its details in the way you want")
